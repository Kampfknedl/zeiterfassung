    ...
    BoxLayout:
        ...
        canvas.before:
            ...
            Rectangle:
                ...
                size: self.size
        Button:
            text: 'Excel Export erstellen'
            size_hint_y: None
            height: '50dp'
            on_release: root.export_excel_with_dialog()
            background_normal: ''
...
class RootWidget(BoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        self._excel_export_path = None  # User-selected Excel export directory
    ...
    def get_saved_excel_path(self):
        """Retrieve saved Excel export path from settings file"""
        try:
            settings_file = os.path.join(self.get_db_dir(), 'excel_settings.txt')
    ...
    def open_export_settings(self):
        ...
        current = self.get_saved_excel_path() or self._excel_export_path or ''
        ...
        def do_pick(*_):
            # Desktop: use tkinter; Android: use SAF via choose_excel_directory
            try:
                ...
                try:
                    self.choose_excel_directory(lambda p: self.on_directory_chosen(p, popup=popup, update_input=path_input))
                except Exception as e:
    ...
    def save_excel_path(self, path):
        """Save Excel export path to settings file"""
        try:
            settings_file = os.path.join(self.get_db_dir(), 'excel_settings.txt')
    ...
    def choose_excel_directory(self, callback):
        """Open Samsung file picker to choose Excel export directory, prioritizing OneDrive"""
        try:
            from jnius import autoclass, cast
            Intent = autoclass('android.content.Intent')
            Uri = autoclass('android.net.Uri')
        ...
        try:
            if request_code == 42 and result_code == -1:  # RESULT_OK = -1
                ...
                if intent is not None:
                    ...
                    self.save_excel_path(uri_string)
    ...
    def export_excel_with_dialog(self):
        """Export Excel to OneDrive if available, otherwise to Documents folder"""
        print("\n" + "="*60)
        print("[EXPORT] ========== EXCEL EXPORT STARTED ==========")
        print("="*60)
        ...
        try:
            ...
            default_filename = f"Zeiterfassung_{selected_customer.replace(' ', '_')}_{timestamp}.xlsx"
            print(f"[EXPORT] Filename: {default_filename}")
            
            # Try saved path first, then OneDrive, then fallback to Documents
            export_dir = self.get_saved_excel_path()
            if not export_dir:
            ...
            try:
                ...
                if filepath:
                    ...
                    self.save_excel_path(export_dir)
                    print(f"[EXPORT] Saved export path for future use")
                    
                    self.export_excel_to_path(export_dir, filename)
                else:
                ...
                # Save for future use
                self.save_excel_path(export_dir)
                self.export_excel_to_path(export_dir, default_filename)
                
        except Exception as e:
            import traceback
            error_msg = f"Fehler beim Excel-Export:\n{str(e)}\n\n{traceback.format_exc()}"
    ...
    def export_excel_to_path(self, export_path, filename=None):
        """Export customer entries as Excel file using openpyxl"""
        print(f"\n[EXPORT] ===== EXPORT_EXCEL_TO_PATH CALLED =====")
        ...
        try:
            if filename is None:
                filename = f"Zeiterfassung_{selected_customer.replace(' ', '_')}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
            ...
            if not filename.endswith('.xlsx'):
                filename = filename.replace('.pdf', '.xlsx').replace('.csv', '.xlsx')
                if not filename.endswith('.xlsx'):
                    filename += '.xlsx'
                print(f"[EXPORT] Converted filename to Excel: {filename}")
            
            excel_path = os.path.join(export_path, filename)
            print(f"[EXPORT] Full Excel path: {excel_path}")

            # Create Excel file using openpyxl
            print(f"[EXPORT] Creating Excel workbook...")
            ...
            ws['A1'] = f'Arbeitsreport: {selected_customer}'
            ...
            if cust and len(cust) > 2 and cust[2]:
                ws[f'A{row_num}'] = 'Adresse:'
                ws[f'B{row_num}'] = cust[2]
                ws[f'A{row_num}'].font = Font(bold=True)
                row_num += 1
            if cust and len(cust) > 3 and cust[3]:
                ws[f'A{row_num}'] = 'Email:'
                ws[f'B{row_num}'] = cust[3]
                ws[f'A{row_num}'].font = Font(bold=True)
                row_num += 1
            if cust and len(cust) > 4 and cust[4]:
                ws[f'A{row_num}'] = 'Telefon:'
                ws[f'B{row_num}'] = cust[4]
                ws[f'A{row_num}'].font = Font(bold=True)
                row_num += 1
            ...
            # Table headers
            row_start = row_num
            headers = ['Datum', 'Tätigkeit', 'Stunden', 'Kommentar', 'Start']
            ...
            print(f"[EXPORT] Writing {len(rows)} entries to Excel...")
            for r in rows:
                date_str = (r[3] or '')[:10]
                activity = r[2] or ''
                duration = float(r[5] or 0)
                notes = r[6] or ''
                start_time = r[3] or ''
                ...
                ws.cell(row=row_num, column=4, value=notes).border = border
                ws.cell(row=row_num, column=5, value=start_time).border = border
            ...
            # Save Excel file
            print(f"[EXPORT] Saving Excel file...")
            wb.save(excel_path)
            
            if os.path.exists(excel_path):
                file_size = os.path.getsize(excel_path)
                print(f"[EXPORT] ✅ Excel successfully created!")
                print(f"[EXPORT] Location: {excel_path}")
                print(f"[EXPORT] Size: {file_size} bytes")
                print(f"[EXPORT] Total hours: {total_hours:.2f}")
                self.show_success_and_open_excel(excel_path, selected_customer, is_uri=False)
            else:
                print(f"[EXPORT] ❌ Excel file not created")
                self.show_error('Export-Fehler', 'Excel-Datei konnte nicht erstellt werden')
    ...
    def show_success_and_open_excel(self, filepath, customer_name, is_uri=False):
        """Show success message and automatically open Excel"""
        try:
            # Track last export for printing/share
            self._last_export_path = filepath
            self._last_export_is_uri = bool(is_uri)
            # Automatically open the Excel
            self.open_excel_file(filepath, is_uri=is_uri)
            ...
            # Check if it's Excel or other
            file_type = 'Excel' if filepath.endswith('.xlsx') else 'Datei'
    ...
    def open_excel_file(self, filepath, is_uri=False):
        ...
        try:
            ...
            if is_uri:
                ...
                mime_type = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' if str(filepath).lower().endswith('.xlsx') else 'application/octet-stream'
    ...
    def export_excel(self, auto_share=False):
        """Legacy Excel export function - redirects to new dialog-based export"""
        self.export_excel_with_dialog()
...
class PoCApp(App):
    def _install_crash_logger(self):
        if getattr(self, "_crash_logger_installed", False):
            return
        ...
        def _hook(exc_type, exc_value, exc_tb):
            ...
            try:
                out_dir = None
                try:
                    if self.root and hasattr(self.root, "get_documents_dir"):
                        out_dir = self.root.get_documents_dir()
                except Exception:
                    out_dir = None

                if not out_dir:
                    out_dir = os.path.join(self.user_data_dir, "logs")

                os.makedirs(out_dir, exist_ok=True)
                crash_file = os.path.join(out_dir, "crash.txt")
                with open(crash_file, "a", encoding="utf-8") as fh:
                    ...
                    fh.write(tb)
                    fh.write("\n")
                print(f"[CRASH] wrote crash log to {crash_file}")
            except Exception as log_err:
                print(f"[CRASH] failed to write crash log: {log_err}")
        ...
        self._crash_logger_installed = True
        print("[APP] Crash logger installed")

    def _lock_display_to_60hz(self):
        """Force the closest supported 60 Hz display mode on Android to avoid hwui mutex crashes seen on high-refresh devices."""
        if platform != "android":
            return
        try:
            from jnius import autoclass
            ...
            if target_mode:
                ...
                attrs.preferredDisplayModeId = target_mode.getModeId()
                window.setAttributes(attrs)
                print(f"[DISPLAY] Preferred display mode set to ~60 Hz (actual {target_mode.getRefreshRate():.1f} Hz)")
        except Exception as e:
            print(f"[DISPLAY] Could not force 60 Hz: {e}")

    def build(self):
        # Cap rendering to 60 FPS to reduce driver load on high-refresh displays (e.g., Galaxy S24 series)
        try:
            from kivy.config import Config
            Config.set('graphics', 'maxfps', '60')
        except Exception as e:
            print(f"[CONFIG] Could not set maxfps: {e}")
    ...
    def on_start(self):
        ...
        # REMOVED: _lock_display_to_60hz() causes hwui mutex crash on Samsung devices
        # self._lock_display_to_60hz()
